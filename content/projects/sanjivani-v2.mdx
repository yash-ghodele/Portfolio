...
image: "/projects/sanjivani-v2.jpg"
iconName: "Sprout"
...
> **This is not a tutorial project**—it's a **portfolio-grade system** showcasing end-to-end full-stack AI engineering with clean architecture, real-world failure modes, and measurable performance.
...
- **Clean Architecture** — Clear separation: AI inference, knowledge base, API layer, frontend
- **Production-Ready APIs** — REST v2 with structured responses, versioning, validation, performance metadata
- **Edge-Optimized AI** — MobileNetV2 with <100ms latency, exportable to `.h5` and `.tflite`
- **Modern Frontend** — Next.js App Router with "Deep Forest + Neon" glassmorphism theme
- **Real Testing Discipline** — 34 automated tests, ~70% coverage, mock-mode CI support
- **Portfolio-First Engineering** — Built to withstand technical interviews, not just demos

---

## 🚀 The Evolution: 1.0 → 2.0
...
- **Model Size:** 14MB (.h5) → 3MB (.tflite compressed)
...
> **IMPORTANT:** Treatment recommendations are 100% **deterministic** from the knowledge base. Gemini only provides explanatory context—it cannot override safety-critical decisions.
...
**→ Detailed breakdown:** [DEPLOYMENT.md](https://github.com/yash-ghodele/Sanjivani-MVP/docs/DEPLOYMENT.md)
...
**Endpoints:**
- `POST /api/v1/detect` — Disease detection (auth required)
- `GET /api/v2/meta/calendar` — Crop lifecycle data (public)
- `GET /api/weather` — Current weather + forecast (public)
- `GET /api/spraying` — Spraying suitability index (public)
...
- **Mitigation:** Confidence <45% → "Uncertain" flag + user warning

**2. Poor Image Quality**
- **Scenario:** Motion blur, extreme backlighting
- **Mitigation:** Low confidence → "Retake Photo" prompt
...
**→ Full analysis:** [FAILURE_MODES.md](https://github.com/yash-ghodele/Sanjivani-MVP/docs/FAILURE_MODES.md)
...
| :--- | :--- | :--- |
| **Validation Accuracy** | **95%+** | 🟢 Production-Ready |
| **Inference Latency** | **10.64ms** | 🟢 Edge-Ready (<50ms) |
| **Model Size** | **9.0 MB** | 🟢 Lightweight |
| **Gemini Latency** | **~1.2s** | 🟢 Fast (<2s target) |
...
2. **Modern Web Engineering** — Next.js App Router, typed APIs, SSR
...
5. **Professional Documentation** — Written for engineers, not tutorials

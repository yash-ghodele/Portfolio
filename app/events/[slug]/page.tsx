...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const { title, subtitle, summary, image, skills } = event
    ...
    return {
        ...
        openGraph: {
            title,
            description: summary,
            url: `https://yash-ghodele.pages.dev/events/${slug}`,
        ...
        alternates: {
            canonical: `https://yash-ghodele.pages.dev/events/${slug}`,
        },
    }
}
        ...
        >
            ...
            <section className="relative w-full min-h-[75vh] md:min-h-[80vh] flex flex-col justify-end overflow-hidden">
                ...
                <div className="absolute inset-0 overflow-hidden">
                    <ParallaxHeroImage src={image || "/placeholder.jpg"} alt={title} />
...
function ParallaxHeroImage({ src, alt }: { src: string, alt: string }) {
    return (
        <Image
            ...
            alt={alt}
            fill
            className="object-cover object-center opacity-50 scale-110 transition-transform duration-100 ease-out"
            style={{ transform: 'translateY(calc(var(--scroll-y, 0) * 0.4px)) scale(1.1)' }}
            priority
            sizes="100vw"
        />
    )
